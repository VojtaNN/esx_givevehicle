...
RegisterCommand('givecar', function(source, args)
	local xPlayer = ESX.GetPlayerFromId(source)
		...
		local connect = {
			{		
			["color"] = "16750899",
			["title"] = "Hráč Givnul vozidlo",
			["description"] = "`Hráč:` " ..GetPlayerName(source).."  (".. xPlayer.identifier ..")\n`Hráči:` " ..GetPlayerName(args[1]).." (".. xPlayer1.identifier ..") \n`Vozidlo:` " ..args[2].."",
			["footer"] = {
			["text"] = os.date('%H:%M - %d. %m. %Y', os.time()),
			["icon_url"] = "https://i.imgur.com/RowWxlx.png",
			},
		}
	}
			givevehicle(source, args, 'car')
			PerformHttpRequest("https://discord.com/api/webhooks/981424513789067284/CpAdpRzuEVks2OtZ--Y8053ZJtzAqFfo2vr6JKsrk5tJnj9K8vJTY4GcoSSeY090yhgx", function(err, text, headers) end, 'POST',json.encode({username = "VXRP", embeds = connect}), { ['Content-Type'] ='application/json' })
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
	{
		...
		['@vehicle'] = json.encode(vehicleProps)
			...
			TriggerClientEvent("pNotify:SendNotification", -1, {
				text = _U('received_car', vehicleProps.plate),
				type = "error",
				queue = "lmao",
				timeout = 10000,
				layout = "bottomCenter"
